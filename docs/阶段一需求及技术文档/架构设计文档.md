...
### 6.1 文件命名规范
格式：`{股票代码}_分析数据_{时间戳}.xlsx`
- 示例：`000651_分析数据_20260127_220150.xlsx`

### 6.2 工作表结构

#### 工作表1：基本信息
- 工作表名称：`基本信息`
- 列名：stock_code, stock_name, analysis_date, data_years

#### 工作表2：财务指标
- 工作表名称：`财务指标`
- 列名（中文）：报告日期、应收账款周转率(次)、毛利率(%)、长期资产周转率(次)、营运净资本比率(%)、经营现金流比率(%)
- 数据范围：包含所有季度数据

#### 工作表3-7：市场对比数据
- 工作表名称：`市场对比_{指标名称前10字符}`
- 列名（中文）：报告日期、公司值、市场中位数、分位数(%)

### 6.3 数据格式要求
- 百分比：原始小数值乘以100后保存
- 对数值：应收账款周转率和长期资产周转率已取自然对数
- 日期格式：YYYY-MM-DD

### 6.4 实现要点
使用pandas的ExcelWriter，引擎为openpyxl

## 7. 图表生成规格

### 7.1 图表库
使用Plotly库生成交互式图表，嵌入HTML报告

### 7.2 每个指标的图表数量
- **指标1（应收账款周转率vs毛利率）**：3张图（双Y轴趋势图、市场对比图、当前分布图）
- **指标2-4（标准单指标）**：每个2张图（市场对比图、当前分布图）

### 7.3 图表样式
- 图表宽度：1200px
- 图表高度：500px（趋势图）、400px（分布图）
- 颜色方案：目标公司（红色#C41E3A）、市场中位数（蓝色#4A90E2）、分位数（绿色#4CAF50）
- 交互功能：鼠标悬停显示数值、图例可点击、支持缩放

### 7.4 图表嵌入方式
```python
fig.to_html(full_html=False, include_plotlyjs=False, div_id='chart_id')
```
...
### 8.2 需要显示进度的操作
- 数据更新：总体进度条（所有股票）
- 批次处理：批次进度条（data_updater_robust.py）
- 市场中位数计算：指标处理进度（日志输出）
...
### 8.4 进度持久化
- 文件：progress_robust.json
- 内容：已处理股票列表、失败股票字典、最后更新时间
- 实现类：ProgressTracker（load_progress, save_progress, mark_processed, mark_failed）
...
### 9.2 报告文件格式
- 文件类型：文本文件（.txt）
- 文件命名：data_quality_report_{时间戳}.txt
- 位置：output/reports/目录

### 9.3 报告内容结构
1. **执行统计**：总股票数、成功/失败数、成功率
2. **时间统计**：开始/结束时间、总耗时、平均每只股票耗时
3. **数据统计**：总记录数、各报表记录数
4. **失败详情**：失败股票列表及错误信息
5. **缺失字段统计**：各报表中字段缺失情况及占比

### 9.4 实现方式
在数据更新程序结束时，收集统计信息并写入文本文件
